...
def forecast_exponential(series, forecast_period):
    ...
    except Exception as e:  # Catching specific exceptions
        print(f"Warning: {e}")  # Log the error for debugging
...
if file:
    ...
    if sales_col is None:
        ...
        try:
            seasonal_value = 'add'
            model = ExponentialSmoothing(df[sales_col], trend='add', seasonal=seasonal_value)
            fit_model = model.fit()
            forecast = fit_model.forecast(forecast_period_sales)
        except Exception as e:
            seasonal_value = None  # Handle fallback
            model = ExponentialSmoothing(df[sales_col], trend='add', seasonal=seasonal_value)
            fit_model = model.fit()
            forecast = fit_model.forecast(forecast_period_sales)
        ...
        # Pie chart Payment analysis
        st.subheader("Payment Method")
        if 'Payment_Method' in df.columns:
            # Drop NaN values & count occurrences
            payment_counts = df['Payment_Method'].value_counts().reset_index()
            payment_counts.columns = ['Payment Method', 'Count']  # Rename columns

            if not payment_counts.empty:  # Ensure we have data
                # Create Pie Chart with Plotly
                fig = px.pie(payment_counts, names='Payment Method', values='Count', title="Distribution of Payment Methods", color_discrete_sequence=px.colors.qualitative.Pastel)

                # Display the chart in Streamlit
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("⚠️ No valid payment method data available.")
        else:
            st.warning("⚠️ 'Payment_Method' column not found in the dataset.")
